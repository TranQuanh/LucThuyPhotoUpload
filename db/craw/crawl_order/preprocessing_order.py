...
def extract_selected_fields(record):
    ...
    result['Mã HĐ'] = record.get('Mã HĐ')[0].get('text')
    # SDT Khách: lấy text đầu tiên nếu là list
    sdt_khach = record.get('SDT Khách')
    if isinstance(sdt_khach, list) and len(sdt_khach) > 0:
        result['SDT Khách'] = sdt_khach[0].get('fullPhoneNum')
    else:
        result['SDT Khách'] = sdt_khach
    return result
